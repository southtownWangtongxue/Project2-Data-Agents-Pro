...
## 阶段 6：流式体验与前端优化（2026-06-26 完成）

- [x] **Analyst 实时流式输出**：LLM 直接输出 Markdown 文本 + 逐 token 推送（消除 5-10s 等待）
- [x] **Clarifier + Planner 合并**：单次 LLM 调用完成意图澄清+执行计划（节省 ~2-3s）
- [x] **Suggestions API 超时降级**：8s 超时→默认模板兜底
- [x] **Redis 检查点 Key 解析修复**：兼容含冒号的 thread_id
- [x] **消息去重**：SQL 仅格式化卡片展示一次；过滤 undefined 字段
- [x] **管道进度实时化**：`_push_node_started` 通过 `push_priority` 直写 merge_queue
- [x] **水平管道 → 可折叠任务清单**：垂直列表 + spinner/checkmark 动画 + 2s 自动折叠
